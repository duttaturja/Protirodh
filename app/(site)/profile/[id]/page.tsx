...
async function getUserProfile(id: string) {
    ...
    try {
        ...
        const reports = await CrimeReport.find({ author: id }).sort({ createdAt: -1 }).populate("author", "name image role").lean();
...
export default async function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="min-h-screen bg-background">
            {/* Header */}
            <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border px-4 py-2 flex items-center gap-4">
            ...
            <div className="px-4 pb-4 relative">
                ...
                <div className="flex justify-end py-3 h-16">
                    {isOwnProfile && (
                        <>
                            {/* Show Verify Button if Unverified */}
                ...
                <div className="mt-2">
                    ...
                    <div className="flex gap-4 mt-3 text-sm">
                        <div><span className="font-bold text-foreground">120</span> <span className="text-muted-foreground">Following</span></div>
                        <div><span className="font-bold text-foreground">450</span> <span className="text-muted-foreground">Followers</span></div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex border-b border-border mt-2">
                <div className="flex-1 text-center py-3 font-bold text-sm border-b-4 border-primary cursor-pointer hover:bg-secondary/30">
                    Posts
                </div>
                <div className="flex-1 text-center py-3 font-medium text-sm text-muted-foreground cursor-pointer hover:bg-secondary/30">
                    Replies
                </div>
                <div className="flex-1 text-center py-3 font-medium text-sm text-muted-foreground cursor-pointer hover:bg-secondary/30">
                    Media
